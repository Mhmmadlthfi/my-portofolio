<template>
  <nav class="fixed w-full top-0 z-50 mix-blend-difference px-6 md:px-12 py-6 flex justify-between items-center">
    ...
    <div class="hidden md:flex gap-8 text-sm font-medium tracking-wide">
      <a 
        href="#home" 
        class="text-white hover:text-neon transition duration-300 uppercase"
      >
        Home
      </a>
      <a 
    ...
    <a 
      href="#contact"
      class="border border-white/20 rounded-full px-5 py-2 text-sm text-white hover:bg-neon hover:text-dark hover:border-neon transition duration-300 uppercase"
    >
      Let's Talk
    </a>
    ...
    <div 
      v-if="isMobileMenuOpen"
      class="absolute top-full left-0 w-full bg-dark/95 backdrop-blur-lg border-t border-white/10 mt-2 md:hidden"
    >
      <div class="flex flex-col px-6 py-4 space-y-4">
        <a 
          href="#home" 
          class="text-white hover:text-neon transition duration-300 uppercase block py-2"
